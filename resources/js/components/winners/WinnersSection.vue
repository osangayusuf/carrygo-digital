    ...
    <section class="mx-auto max-w-[1300px] px-4 pt-8 pb-20">
        <header class="mb-10">
            <div class="space-y-2">
                ...
                <p class="text-sm text-muted-green">Verified winners from completed auctions on Bidora.</p>
            </div>
        ...
        <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            <article v-for="winner in winners.data" :key="winner.id"
                ...
                <div class="flex flex-1 flex-col p-4">
                    ...
                    <div v-if="winner.review" class="mb-4 pt-3 border-t border-sage-border">
                        <div class="flex items-center justify-between gap-1 mb-1.5">
                            <div class="flex items-center gap-0.5">
                                <span v-for="star in 5" :key="star" class="material-symbols-outlined text-[14px]"
                                    :class="star <= Math.round(winner.review.rating) ? 'text-lemon fill-1' : 'text-sage-mid'">
                                    star
                                </span>
                            </div>
                            <span v-if="winner.review.social_handle"
                                class="text-[9px] font-bold text-forest bg-sage-bg px-1.5 py-0.5 rounded">
                        ...
                        <div v-if="winner.review.photos?.length || winner.review.video"
                            class="mt-2 flex items-center gap-1.5">
                            <div v-for="(photo, idx) in winner.review.photos" :key="idx"
                            ...
                            <div v-if="winner.review.video"
                                ...
                                <span
                                    class="material-symbols-outlined text-white text-[12px] absolute z-10">play_arrow</span>
                    ...
                    <div class="mt-auto flex items-center justify-between gap-2">
                        ...
                        <Link v-if="winner.bid" :href="auctionShow.url(winner.bid)"
