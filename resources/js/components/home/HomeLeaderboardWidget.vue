...
<template>
    <aside class="sticky top-20 overflow-hidden rounded-xl border-2 border-sage-border bg-navy/95 font-sans">

        <!-- Header -->
        <div
            class="flex items-center justify-between border-b border-sage-border bg-linear-to-br from-forest to-navy px-4 py-3">
            <div class="flex items-center gap-2">
                <span class="material-symbols-outlined text-2xl text-lemon">emoji_events</span>
                <div>
                    <p class="text-base font-black uppercase tracking-widest text-lemon">Weekly Leaderboard</p>
        ...
        <div v-if="!topBidders" class="py-1.5">
            <div v-for="n in 5" :key="n"
                ...
                <div class="flex flex-1 flex-col gap-1">
                    <div class="h-2.5 w-3/4 animate-pulse rounded bg-white/10"></div>
                    <div class="h-2 w-2/5 animate-pulse rounded bg-white/6" style="animation-delay:0.3s"></div>
                </div>
            </div>
        </div>

        <!-- Empty state -->
        <div v-else-if="topBidders.length === 0" class="flex flex-col items-center gap-1.5 px-4 py-8 text-center">
            <span class="material-symbols-outlined text-4xl text-sage-border">leaderboard</span>
            <p class="text-[12px] font-bold text-sage-light">No bids this week yet.</p>
            <p class="text-[11px] text-sage-dark">Be first on the board!</p>
        </div>

        <!-- Bidder rows -->
        <ul v-else class="py-1.5">
            <li v-for="bidder in topBidders" :key="bidder.rank"
                ...
                <div class="min-w-0 flex-1">
                    <p class="truncate font-extrabold leading-snug text-sage-light">
                        {{ bidder.name }}
                    </p>
                    <p class="text-xs leading-snug text-sage-dark">{{ bidder.msisdn }}</p>
                </div>

                <!-- Points -->
                <div class="text-right">
                    <p class="text-xs font-black leading-tight text-lemon">
                        {{ bidder.total_bid_pts.toLocaleString() }}
                    </p>
                    <p class="text-xs font-bold uppercase tracking-wide text-sage-dark">pts</p>
                </div>
            </li>
        </ul>
