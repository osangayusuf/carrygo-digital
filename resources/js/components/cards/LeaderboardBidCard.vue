...
<template>
    ...
    >
        <div class="relative h-36 cursor-pointer overflow-hidden bg-sage-bg" @click="expandedImage = bid.image">
            <img
                v-if="bid.image"
                :src="bid.image"
                :alt="bid.name"
        ...
        <div class="flex flex-1 flex-col p-3">
            <div class="mb-2 flex items-start justify-between">
                <h2 class="line-clamp-2 pr-2 text-xs font-extrabold text-ink">{{ bid.name }}</h2>
            ...
            <div class="mt-auto">
                <h3 class="mb-2 text-[9px] font-black tracking-widest text-muted-green uppercase">Top Bidders</h3>

                <div v-if="bid.top_bidders.length > 0" class="flex flex-col gap-1.5">
                    <div
                        v-for="(bidder, index) in bid.top_bidders"
                        :key="index"
                        class="flex items-center justify-between rounded-xl px-2 py-1.5 sm:px-2.5"
                        :class="index === 0 ? 'border border-sage-border bg-sage-bg' : 'bg-sage-bg/50'"
                    >
                        <div class="flex items-center gap-1.5 sm:gap-2">
                            <span
                                class="flex h-4 w-4 items-center justify-center rounded-full text-[8px] font-black"
                                :class="
                                    index === 0 ? 'bg-navy text-lemon shadow-md shadow-navy/30' : 'bg-white text-ink'
                                "
                            >
                                {{ index + 1 }}
                            </span>
                ...
                <div v-else class="rounded-xl border border-dashed border-sage-border p-3 text-center sm:p-4">
                    <p class="text-[10px] font-bold text-muted-green sm:text-xs">No bids recorded</p>
                    <p class="mt-0.5 text-[9px] text-muted-green/80 sm:text-[10px]">
