...
function goToPage(page: number): void {
    router.get(walletRoute.url({ query: { page } }), {}, { preserveState: true, preserveScroll: true, replace: true });
}
</script>

<template>
    <section class="mt-8">
        <h3 class="mb-4 text-lg font-semibold text-on-surface">Transaction history</h3>

        <div
            v-if="transactions.data.length === 0"
            class="rounded-xl border border-dashed border-outline-variant bg-surface-container-low p-10 text-center"
        >
            <span class="material-symbols-outlined mb-3 text-4xl text-on-surface-variant">receipt_long</span>
        ...
        <div v-else class="overflow-hidden rounded-xl border border-outline-variant">
            <div class="hidden md:block">
                <table class="w-full text-left text-sm">
                    <thead class="border-b border-outline-variant bg-surface-container-low">
                        <tr>
                            <th class="px-4 py-3 text-xs font-bold uppercase tracking-wide text-on-surface-variant">
                                Type
                            </th>
                            <th class="px-4 py-3 text-xs font-bold uppercase tracking-wide text-on-surface-variant">
                                Auction
                    ...
                    <tbody>
                        ...
                        >
                            <td class="px-4 py-3">
                                <TransactionTypeBadge :type="tx.type" :label="tx.type_label" />
                            </td>
                            ...
                            <td class="px-4 py-3 font-bold text-on-surface">
                                {{ amountPrefix(tx.direction) }}{{ tx.amount.toLocaleString() }} pts
                                <span
            ...
            <div class="divide-y divide-outline-variant md:hidden">
                <article v-for="tx in transactions.data" :key="tx.id" class="bg-surface-container-lowest p-4">
                    ...
                    <p v-if="tx.auction_id" class="mt-2 text-xs text-on-surface-variant">
                        Auction
                        <Link
                            :href="auctions.show.url(tx.auction_id)"
                            class="font-medium text-forest hover:underline"
                    ...
                    <p v-if="tx.naira_amount" class="text-xs text-on-surface-variant">
                        ₦{{ tx.naira_amount.toLocaleString() }}
                    </p>
                    <p class="mt-1 text-xs text-on-surface-variant">{{ formatDate(tx.created_at) }}</p>
                </article>
            </div>
        </div>
