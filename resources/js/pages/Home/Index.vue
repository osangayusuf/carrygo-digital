...
import { trending, openBids as openBidsRoute, tasks, howToPlay } from '@/routes/index';
...
<template>
    ...
    <div class="overflow-x-hidden bg-sage-bg text-left font-sans text-ink">
        ...
        <FeaturedAuctionsCarousel v-if="props.featuredBids && props.featuredBids.length > 0" :bids="props.featuredBids"
            :user-points="props.userPoints ?? null" />

        <!-- HYPE MARQUEE BANNER -->
        <div class="my-2 overflow-hidden border-y-2 border-lemon bg-linear-to-r from-forest to-navy py-2">
        ...
        <div class="mx-auto mt-4 max-w-[1300px] px-4">
            <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:gap-6">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <!-- LIVE OPPORTUNITIES -->
                    <div v-if="props.openBids && props.openBids.length > 0" class="mb-5">
                        <div class="mb-3.5 flex items-center justify-between">
                            <div class="flex items-center font-condensed text-2xl font-extrabold text-ink">
                                <span class="relative mr-2.5 flex h-3 w-3 align-middle">
                                    <span
                            ...
                            <Link :href="openBidsRoute.url()" class="text-sm font-bold text-forest hover:underline">View
                                All →</Link>
                        </div>
                        <div class="grid grid-cols-2 gap-2.5 sm:grid-cols-3">
                            <BidCard v-for="bid in props.openBids.slice(0, 6)" :key="bid.id" :bid="bid"
                                :user-points="props.userPoints ?? null" />
                        </div>
                    </div>
                    ...
                    <div v-if="hasActiveSearch && !hasAnyBids" class="mb-5">
                        <div
                            ...
                            <p class="mt-2 text-sm text-muted-green">
                                Nothing matched "<span class="font-bold text-ink">{{ props.search }}</span>". Try a
                                different keyword or browse all categories below.
                            </p>
                        </div>
                    </div>

                    <!-- PRODUCT STRIP -->
                    <div v-if="props.bids && props.bids.length > 0" class="grid grid-cols-2 gap-2.5 sm:grid-cols-3">
                        <BidCard v-for="bid in props.bids.slice(0, 9)" :key="bid.id" :bid="bid"
                            :user-points="props.userPoints ?? null" />
                    </div>
                ...
                <div class="w-full shrink-0 lg:w-md">
                    <HomeLeaderboardWidget :top-bidders="props.homeTopBidders" />
                </div>
        ...
        <div class="my-3 overflow-hidden border-y-2 border-lemon bg-navy py-2"
            ...
            <div class="flex animate-marquee items-center whitespace-nowrap">
                <span class="inline-flex items-center gap-2 px-7 text-sm font-bold text-lemon" v-for="bid in props.bids"
                    :key="bid.id">
                    <span class="inline-block h-2 w-2 rounded-full bg-lemon"></span>
                    {{ bid.name }}
                    <span class="font-extrabold text-white">{{
                        formatPrice(bid.price)
                        }}</span>
                    <span class="ml-1 rounded-sm bg-forest px-2 py-0.5 text-xs font-extrabold text-lemon"
        ...
        <!-- CATEGORIES -->
        <div class="mx-auto mb-5 max-w-[1300px] px-4" v-if="props.categories && props.categories.length > 0">
            <div class="mb-3.5 flex items-center justify-between">
                <div class="flex items-center font-condensed text-2xl font-extrabold text-ink">
                    <span class="mr-2 inline-block h-[22px] w-1 rounded-sm bg-forest align-middle"></span>
                    Browse Categories
                </div>
                <Link :href="trending.url()" class="text-sm font-bold text-forest hover:underline">View All →</Link>
            </div>
            <div class="grid grid-cols-3 gap-2.5 sm:grid-cols-4 lg:grid-cols-8">
                <Link :href="trending.url() + '?category=' + encodeURIComponent(cat)
                    ...
                    <div class="text-xs font-extrabold text-ink group-hover:text-lemon">
                        {{ cat }}
                    </div>
                </Link>
            </div>
        ...
        <!-- TRENDING BIDS -->
        <div class="mx-auto mb-5 max-w-[1300px] px-4" v-if="props.trendingBids && props.trendingBids.length > 0">
            <div class="mb-3.5 flex items-center justify-between">
                <div class="flex items-center font-condensed text-2xl font-extrabold text-ink">
                    <span class="mr-2 inline-block h-[22px] w-1 rounded-sm bg-forest align-middle"></span>
                ...
                <Link :href="trending.url()" class="text-sm font-bold text-forest hover:underline">View More →</Link>
            </div>
            <div class="grid grid-cols-2 gap-2.5 md:grid-cols-5">
                <BidCard v-for="bid in props.trendingBids.slice(0, 8)" :key="bid.id" :bid="bid"
                    :user-points="props.userPoints ?? null" />
            </div>
        ...
        <div class="mx-auto mb-5 max-w-[1300px] px-4"
            ...
            <div class="mb-3.5 flex items-center justify-between">
                ...
                <Link :href="trending.url() + '?sort=recent'" class="text-sm font-bold text-forest hover:underline">View
                    More →</Link>
            </div>
            <div class="grid grid-cols-2 gap-2.5 md:grid-cols-5">
            ...
            <div class="grid grid-cols-1 gap-3.5 lg:grid-cols-2">
                <div
                    ...
                    <div>
                        ...
                        <div class="mb-1.5 font-condensed text-3xl leading-tight font-black text-white">
                            Win More<br />Points Free!
                        </div>
                        <div class="mb-3.5 text-xs leading-snug text-[#cde] opacity-85">
                    ...
                    <div class="text-[90px] leading-none font-black opacity-[0.18]">
        ...
        <!-- PREMIUM & LUXURY -->
        <div class="mx-auto mb-5 max-w-[1300px] px-4" v-if="props.luxuryBids && props.luxuryBids.length > 0">
            <div class="mb-3.5 flex items-center justify-between">
                <div class="flex items-center font-condensed text-2xl font-extrabold text-ink">
                    <span class="mr-2 inline-block h-[22px] w-1 rounded-sm bg-forest align-middle"></span>
                ...
                <Link :href="trending.url() + '?sort=price'" class="text-sm font-bold text-forest hover:underline">View
                    All →</Link>
            </div>
            <div class="grid grid-cols-2 gap-2.5 md:grid-cols-5">
        ...
        <div class="mb-5 overflow-hidden border-y-2 border-lemon bg-navy py-4">
            <div class="flex animate-marquee items-center whitespace-nowrap">
                ...
                <div class="inline-flex items-center gap-3 px-10">
                    ...
                    <div class="text-left">
                        <div class="text-xs font-black tracking-wider text-[#8aaa80] uppercase">
                            Step 1
                        </div>
            ...
            <div v-for="(bids, categoryName) in props.categoryBids" :key="String(categoryName)"
                ...
                <template v-if="bids && bids.length > 0">
                    <div class="mb-3.5 flex items-center justify-between">
                        <div class="flex items-center font-condensed text-2xl font-extrabold text-ink">
                            <span class="mr-2 inline-block h-[22px] w-1 rounded-sm bg-forest align-middle"></span>
                            <span class="material-symbols-outlined mr-1.5 text-[22px] leading-none text-forest">{{
                        ...
                        <Link :href="trending.url() +
                            ...
                            " class="text-sm font-bold text-forest hover:underline">View All →</Link>
                    </div>
                    <div class="grid grid-cols-2 gap-2.5 md:grid-cols-5">
