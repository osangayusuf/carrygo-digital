...
<template>
    ...
    <SupportLayout :breadcrumbs="[{ title: 'Tickets' }]">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 flex flex-col gap-4">
                    ...
                    <div class="flex border-b border-outline-variant/50">
                        <button @click="switchTab('unassigned')" :class="[
                        ...
                        <button @click="switchTab('mine')" :class="[
                            'px-5 py-3 font-bold uppercase tracking-wider transition-all border-b-2 text-[10px]',
                            activeTab === 'mine'
                                ? 'border-primary text-primary'
                                : 'border-transparent text-on-surface-variant hover:text-on-surface'
                        ...
                        <button @click="switchTab('closed')" :class="[
                            'px-5 py-3 font-bold uppercase tracking-wider transition-all border-b-2 text-[10px]',
                            activeTab === 'closed'
                                ? 'border-primary text-primary'
                                : 'border-transparent text-on-surface-variant hover:text-on-surface'
                    ...
                    <!-- Tickets List -->
                    <div
                        class="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm flex flex-col overflow-hidden">
                        <div class="divide-y divide-outline-variant/40">
                            <div v-for="ticket in tickets.data" :key="ticket.id"
                                ...
                                <div class="space-y-1.5 flex-1 min-w-0">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        <span
                                            class="font-bold text-[10px] uppercase text-primary-container bg-primary/80 border border-primary/20 px-2 py-0.5 rounded">
                                            {{ ticket.category }}
                                        </span>
                                        <span
                                            :class="['text-[9px] font-black uppercase px-2 py-0.5 rounded border', getPriorityClass(ticket.priority)]">
                                            {{ ticket.priority }}
                                        </span>
                                        <span
                                            :class="['text-[9px] font-black uppercase px-2 py-0.5 rounded border', getStatusClass(ticket.status)]">
                                            {{ ticket.status }}
                                        </span>
                                    </div>
                                    <h2
                                    ...
                                    <div
                                        class="flex items-center gap-3 text-[10px] text-on-surface-variant flex-wrap font-medium">
                                        ...
                                        <template v-if="ticket.agent">
                                            <span>•</span>
                                            <span
                                                class="text-secondary font-bold bg-secondary/10 px-2 py-0.5 rounded uppercase tracking-wider text-[9px]">
                                ...
                                <div class="flex items-center self-end md:self-auto">
                                    <Link :href="ticketsShow.url(ticket.id)"
                            ...
                            <div v-if="tickets.data.length === 0"
                                class="py-16 text-center flex flex-col items-center justify-center gap-3 text-on-surface-variant">
                                ...
                                <span class="text-xs max-w-xs text-on-surface-variant/75">There are no tickets in this
                                    queue category.</span>
                            </div>
                        </div>
                        ...
                        <div v-if="tickets.last_page > 1"
                            class="px-6 py-4 bg-surface-container-low border-t border-outline-variant/40 flex justify-between items-center">
                ...
                <div
                    class="bg-surface-container-lowest border border-outline-variant p-6 rounded-xl shadow-sm space-y-4 h-fit">
                    <h2
                        class="text-sm font-bold uppercase tracking-wider text-primary border-b border-outline-variant/30 pb-3">
                        Workstation Overview</h2>
                    <div class="space-y-3.5 text-xs text-on-surface-variant leading-relaxed">
                        <p>Welcome to the bidora helpdesk. To claim a ticket or reply to a customer request, open any
                            ticket card in the
                            queue list.</p>
                        <div class="border border-outline-variant/50 p-4 rounded-lg bg-surface-container-low space-y-2">
                            <p
                                class="font-bold text-primary uppercase text-[9px] tracking-wider flex items-center gap-1.5">
        ...
        <div v-if="showCreateModal"
            ...
            <div
                class="bg-surface-container-lowest border border-outline-variant w-full max-w-lg p-6 flex flex-col gap-4 rounded-xl shadow-xl">
                ...
                <form @submit.prevent="submitCreate" class="space-y-4 text-xs">
                    ...
                    <div class="flex flex-col gap-1.5">
                        <label class="text-on-surface-variant uppercase tracking-wider text-[10px] font-bold">Select
                            Customer</label>
                        <select v-model="createForm.customer_id" required
                            ...
                            <option v-for="c in customers || []" :key="c.id" :value="c.id">{{ c.name }} ({{ c.email }})
                            </option>
                        </select>
                        <p v-if="createForm.errors.customer_id" class="text-error mt-1">{{ createForm.errors.customer_id
                            }}</p>
                    </div>
                    ...
                    <div class="flex flex-col gap-1.5">
                        <label for="subject"
                            class="text-on-surface-variant uppercase tracking-wider text-[10px] font-bold">Subject</label>
                        <input id="subject" v-model="createForm.subject" type="text" required
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="flex flex-col gap-1.5">
                            ...
                            <p v-if="createForm.errors.category" class="text-error mt-1">{{ createForm.errors.category
                                }}</p>
                        </div>
                        ...
                        <div class="flex flex-col gap-1.5">
                            ...
                            <select v-model="createForm.priority" required
                                class="bg-surface-container-low border border-outline-variant text-on-surface px-4 py-3 rounded-lg focus:outline-none focus:ring-1 focus:ring-secondary font-bold uppercase">
                                <option v-for="prio in priorities" :key="prio" :value="prio">{{ prio }}</option>
                            </select>
                            <p v-if="createForm.errors.priority" class="text-error mt-1">{{ createForm.errors.priority
                                }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col gap-1.5">
                        <label for="body"
                            class="text-on-surface-variant uppercase tracking-wider text-[10px] font-bold">Message
                            Details</label>
                        <textarea id="body" v-model="createForm.body" rows="5" required
                            placeholder="PROVIDE DETAILS ON THE INQUIRY..."
                            class="bg-surface-container-low border border-outline-variant text-on-surface p-4 rounded-lg focus:outline-none focus:ring-1 focus:ring-secondary font-medium"></textarea>
