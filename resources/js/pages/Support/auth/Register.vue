...
<template>
    <Head title="Agent Registration - CarryGo Support" />

    <div class="relative flex min-h-screen items-center justify-center overflow-y-auto bg-surface px-6 py-12">
        ...
        <div class="relative z-10 w-full max-w-lg my-8">
            ...
            <div class="rounded-2xl border border-outline-variant/20 bg-surface-container-lowest p-8 shadow-[0_20px_60px_rgba(13,27,42,0.10)]">
                ...
                <Form :action="registerStore.url()" method="post" :reset-on-success="['password', 'password_confirmation']" v-slot="{ errors, processing }">
                    <div class="space-y-5">
                        ...
                        <div class="space-y-1.5">
                            <label for="name" class="ml-0.5 block text-xs font-bold tracking-widest text-on-surface-variant uppercase">
                                Full Name
                            </label>
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <!-- Phone -->
                            <div class="space-y-1.5">
                                <label for="phone" class="ml-0.5 block text-xs font-bold tracking-widest text-on-surface-variant uppercase">
                                    Phone Number
                                </label>
                        ...
                        <!-- Department & Employee ID Grid -->
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <!-- Department -->
                            <div class="space-y-1.5">
                                <label for="department" class="ml-0.5 block text-xs font-bold tracking-widest text-on-surface-variant uppercase">
                                    Department
                                </label>
                                <input id="department" name="department" type="text" required
                                    class="block w-full rounded-lg border-none bg-surface-container-low px-5 py-3.5 text-sm text-on-surface shadow-sm transition-all placeholder:text-outline/50 focus:ring-2 focus:ring-secondary/40 focus:outline-none"
                                    placeholder="Customer Operations" />
                                <p v-if="errors.department" class="mt-1 text-xs font-medium text-error">{{ errors.department }}</p>
                            </div>

                            <!-- Employee ID -->
                            <div class="space-y-1.5">
                                <label for="employee_id" class="ml-0.5 block text-xs font-bold tracking-widest text-on-surface-variant uppercase">
                                    Employee ID
                                </label>
                                <input id="employee_id" name="employee_id" type="text" required
                                    class="block w-full rounded-lg border-none bg-surface-container-low px-5 py-3.5 text-sm text-on-surface shadow-sm transition-all placeholder:text-outline/50 focus:ring-2 focus:ring-secondary/40 focus:outline-none"
                                    placeholder="EMP-00123" />
                                <p v-if="errors.employee_id" class="mt-1 text-xs font-medium text-error">{{ errors.employee_id }}</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            ...
                            <div class="space-y-1.5">
                                <label for="password_confirmation" class="ml-0.5 block text-xs font-bold tracking-widest text-on-surface-variant uppercase">
                                    Confirm Password
                                </label>
                                <input id="password_confirmation" name="password_confirmation" :type="showPassword ? 'text' : 'password'" required
                                    class="block w-full rounded-lg border-none bg-surface-container-low px-5 py-3.5 text-sm text-on-surface shadow-sm transition-all placeholder:text-outline/50 focus:ring-2 focus:ring-secondary/40 focus:outline-none"
                                    placeholder="••••••••" />
                                <p v-if="errors.password_confirmation" class="mt-1 text-xs font-medium text-error">{{ errors.password_confirmation }}</p>
                            </div>
                        </div>

                        <button type="submit" :disabled="processing"
                            class="flex w-full items-center justify-center gap-2 rounded-lg bg-navy px-4 py-3.5 text-sm font-extrabold text-lemon shadow-lg shadow-navy/20 transition-all hover:bg-forest active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60 cursor-pointer mt-4">
                ...
                <div class="mt-6 text-center text-xs text-on-surface-variant">
                    Already registered?
                    <Link :href="login.url()" class="font-bold text-primary hover:underline">
                        Sign in to Portal
                    </Link>
                </div>
            </div>
