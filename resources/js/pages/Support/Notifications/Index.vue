        ...
        <div class="flex flex-col gap-6 font-sans text-xs">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-on-surface-variant mt-1">Stay updated with live chat sessions, ticket
                        assignments, and replies.</p>
                </div>
                ...
                <div class="divide-y divide-outline-variant/40">
                    <div v-for="item in list" :key="item.id" :class="[
                        ...
                        <div class="flex items-start gap-4 flex-1 min-w-0">
                            <!-- Icon -->
                            <div
                                :class="['w-8 h-8 rounded-lg flex items-center justify-center shrink-0 shadow-sm', getIconBgClass(item.icon)]">
                            ...
                            <div class="space-y-1 flex-1 min-w-0">
                                <div class="flex items-center gap-2 flex-wrap">
                                    <h2
                                        :class="['text-xs truncate transition-colors group-hover:text-primary', !item.read_at ? 'font-extrabold text-on-surface' : 'font-bold text-on-surface-variant']">
                                        {{ item.title }}
                                    </h2>
                                    <span v-if="!item.read_at"
                                        class="bg-primary/10 text-primary border border-primary/25 text-[8px] font-black uppercase px-1.5 py-0.5 rounded">
                                        New
                                    </span>
                                </div>
                                <p
                                    :class="['text-[11px] leading-relaxed', !item.read_at ? 'font-bold text-on-surface/90' : 'font-medium text-on-surface-variant']">
                                    {{ item.body }}
                                </p>
                                <div
                                    class="flex items-center gap-1.5 text-[9px] text-on-surface-variant/70 font-semibold">
                        ...
                        <div class="flex items-center gap-2 self-end md:self-auto">
                            <button v-if="!item.read_at" @click="markRead(item.id)"
                    ...
                    <div v-if="list.length === 0"
                        ...
                        <span class="text-xs max-w-xs text-on-surface-variant/75">You have no portal notifications at
                            this time.</span>
                    </div>
                </div>
            </div>
